...
func _physics_process(delta: float) -> void:
	...
	#checks every step if the joint between bones of different bots is to be broken
		...
		if (jointAngleDif > (180-AngleVariationToBreakJoint)) and (velAngleDif > (180-AngleVariationToBreakJoint)):
			#if the movement direction is opposite the joint direction
